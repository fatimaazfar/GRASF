...
Every new AI coding session starts cold. GRAASF fixes that. It attaches a persistent knowledge graph to your project, extracts structured knowledge from Claude Code sessions, and injects the right context back at the start of the next one — automatically, in the background, without changing how you work.
...
That's it. Open Claude Code normally. GRAASF runs in the background from here.

`graasf init` scans your codebase, builds an initial knowledge graph, installs Claude Code hooks, and writes a `CLAUDE.md` to your repo root. Claude Code reads `CLAUDE.md` automatically at the start of every session.
...
Type any of these in a Claude Code prompt and GRAASF records them immediately, before the AI even sees your message:

| Annotation | What it records |
|---|---|
| `graasf:decision Use RS256 for JWT — needed for gateway verification` | A pinned architectural decision. Never decays. |
| `graasf:goal Refactor the auth module to be stateless` | The intent for this session. Appears in the next session's context. |
| `graasf:note Token TTL is 15 minutes per security policy` | A plain note node in the graph. |
| `graasf:dead-end Redis session store — adds infra dependency, abandoned` | A path you ruled out. Shows in Remnant traces so you don't revisit it. |
...
GRAASF auto-detects which layer to use based on what's available. You can force a layer with the `GRASF_LAYER` environment variable.
...
**Layer 0 is honest about what it gives you.** It tracks your codebase and whatever you annotate. It does not read conversations. If you want GRAASF to understand *why* you made a change — not just *what* changed — use Layer 1 or Layer 2.
...
Uses `claude-haiku-4-5` — the cheapest, fastest Anthropic model. GRAASF sends the session transcript once at session end, not per-message. A typical session costs $0.01–0.03.
...
GRAASF checks `http://localhost:11434` at session end and selects the best available model in this order: `mistral:7b`, `llama3.2:3b`, `llama3:8b`, `phi3:mini`. No API key needed.
...
```bash
graasf init                     # Scan repo, build initial graph, install hooks
graasf status                   # Show graph stats and active layer
graasf query <text>             # FTS5 search across the graph
graasf inject                   # Regenerate CLAUDE.md and AGENTS.md from current graph
graasf inject --stdout          # Print context slice to stdout
graasf inject --format claude   # Regenerate only CLAUDE.md
graasf inject --format agents   # Regenerate only AGENTS.md
graasf hooks install            # (Re)install Claude Code hooks
graasf hooks uninstall          # Remove GRAASF hooks
graasf session list             # List sessions with goal summaries
graasf session show <id>        # Show full session record
graasf decay run                # Manually trigger a decay pass
graasf gc                       # Archive remnants older than 30 days
graasf config set <key> <val>   # Set a config value
graasf config show              # Show current config
graasf watch                    # Start file-change watcher (daemon, Layer 0)
```
...
GRAASF stores per-project config at `.grasf/config.json`. The defaults work for most projects. Change them with `graasf config set <key> <value>`.
...
## What GRAASF writes to your project
...
│   └── annotations.js     # graasf:* annotation parser
├── graph/
